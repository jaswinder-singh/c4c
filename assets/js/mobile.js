...
Weebly.mobile_navigation = (function($) {
    ...
    */
    ...
    var menuTap = function(e) {
        ...
        if (navStyle === SLIDE_DOWN) {
            ...
            var $pageWrapper = $('#c4c-page-wrapper');
            ...
            cb = function() {
                ...
                if (!isOpen) {
                    var $backgroundImage = (Weebly.mobile) ? $('#w-fixed-bg') : $('.c4c-background');
    ...
    var tweenMenu = function($oldSlide, $newSlide, rightToLeft) {
        var $animContainer = $('.c4c-animation-wrap', $menu),
  ...
  /**
    ...
    var resizeMenu = function() {
        ...
        var menuHeight = $menu.find('.c4c-menu-slide:visible').outerHeight();
        if (menuHeight > 0) {
    ...
    var initBackground = function() {
        ...
        $body.prepend('<div class="c4c-background"></div>');
        var $backgroundImage = $body.find('.c4c-background');
    ...
    var initMenu = function() {
        var $sideMenus = $('#navmobile .c4c-menu-wrap'),
            $headerUl, $sliderContainer;

        $menuButton = $('#nav-trigger');
        $headerUl = $('#navmobile .c4c-menu-default');

        $headerUl.wrap('<div class="c4c-mobile-menu" />');
        $menu = $headerUl.up('.c4c-mobile-menu');
        ...
        $headerUl.addClass('c4c-menu-slide').css(slideCSS);
        $sideMenus.addClass('c4c-menu-slide').css(slideCSS);
        ...
        $menu.append('<div class="c4c-animation-wrap"></div>');
        $sliderContainer = $('.c4c-animation-wrap', $menu);
        ...
        $sideMenus.each(function() {
            var $sideMenu  = $(this),
                ...
                $previousMenu = $sideMenu.parents('.c4c-menu-slide');

            var $backLink = $('<li class="c4c-menu-back-item">'
                    +   '<a><span class="c4c-menu-mobile-arrow"></span><span class="c4c-menu-back">Back</span></a>'
                + '</li>');
            ...
            var $rootLink = $('<li class="c4c-menu-master-item"></li>').html($sideMenuRoot);          

            $ul.prepend($rootLink);
            $ul.prepend($backLink);

            $parentAnchor.removeAttr('href').removeAttr('onclick');
            $parentAnchor.append('<span class="c4c-menu-mobile-arrow"></span>');
        ...
        if ($body.hasClass('c4c-menu-dropdown')) {
            initSlideDown();
        } else if ($body.hasClass('c4c-menu-slideright')) {
            initSlideRight();
        }

        // prefer active state classes over html active
        if (supportsTouch) {
            addActiveState($('.c4c-home-link'));
    ...
    var initSlideRight = function() {
        ...
        $body.prepend('<div id="c4c-viewport" ><div id="c4c-page-wrapper" /></div>');
        var $pageWrapper = $('#c4c-page-wrapper');
    ...
    $(function() {
        ...
        if (window.FastClick) {
            FastClick.attach($('.c4c-menu-button')[0]);
            FastClick.attach($('.c4c-mobile-menu')[0]);
        }
